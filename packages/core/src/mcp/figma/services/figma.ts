...
import { fetchWithRetry, fetchTextWithRetry } from "../utils/fetch-with-retry.js";
import { FIGMA_PLUGIN_ID } from "../utils/constants.js";
...
export class FigmaService {
  ...
  /**
   * Fetch actual SVG markup for a set of nodes via the Figma images/export API.
   *
   * Nodes whose exported SVG exceeds `maxContentLength` are silently skipped
   * (they are likely raster-image containers, not icons).
   *
   * @returns Map of nodeId → SVG string
   */
  async fetchSvgContents(
    fileKey: string,
    nodeIds: string[],
    ...
    const svgUrls = await this.getNodeRenderUrls(fileKey, nodeIds, "svg");
      ...
      Object.entries(svgUrls).map(async ([nodeId, url]) => {
        try {
          ...
          if (content.length <= maxContentLength) {
            ...
            Logger.log(
              `SVG for node ${nodeId} exceeds ${maxContentLength} chars (${content.length}), skipped`,
            );
          }
        } catch (error) {
          Logger.log(`Failed to fetch SVG for node ${nodeId}: ${error}`);
        }
      }),
    );
   ...
   * Includes plugin_data so pluginData written by this plugin is returned.
   */
  async getRawFile(fileKey: string, depth?: number | null): Promise<GetFileResponse> {
    const cacheKey = `file:${fileKey}:${depth ?? ''}`;
    
    if (this.responseCache.has(cacheKey)) {
      Logger.log(`Cache hit for Figma file: ${fileKey}`);
      return this.responseCache.get(cacheKey);
    }

    const params = new URLSearchParams({ plugin_data: FIGMA_PLUGIN_ID });
  ...
  async getRawNode(
    ...
    if (this.responseCache.has(cacheKey)) {
      Logger.log(`Cache hit for Figma node: ${nodeId} from ${fileKey}`);
      return this.responseCache.get(cacheKey);
    }

    const params = new URLSearchParams({
      ids: nodeId,
      plugin_data: FIGMA_PLUGIN_ID,
    });
    if (depth != null) params.set("depth", String(depth));
