...
/**
 * Fetch a URL and return the response body as plain text (with curl fallback).
 */
export async function fetchTextWithRetry(url: string): Promise<string> {
  try {
    ...
    Logger.log(
      `[fetchTextWithRetry] Fetch failed for ${url}: ${fetchError.message}. Trying curl fallback.`,
    );
    try {
      const { stdout } = await execFileAsync("curl", ["-s", "-S", "--fail-with-body", "-L", url]);
      if (!stdout) {
        throw new Error("Curl command returned empty stdout.");
      }
      return stdout;
    } catch (curlError: any) {
      Logger.error(`[fetchTextWithRetry] Curl fallback also failed: ${curlError.message}`);
      throw fetchError;
    }
  }
}
