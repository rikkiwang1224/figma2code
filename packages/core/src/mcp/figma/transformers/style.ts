...
/**
 * Normalized image fill with CSS properties and processing metadata
 *
...
export type NormalizedImageFill = {
  ...
  /**
   * Ready-to-use placeholder image URL (e.g. "https://placehold.co/200x100").
   * Dimensions are derived from the owning Figma node's absoluteBoundingBox.
   * Code generation should use this URL for mock / placeholder images.
   */
...
export type NormalizedPatternFill = {
  type: "PATTERN";
  patternSource: {
    /**
...
export type NormalizedFill =
  ...
  | NormalizedPatternFill
  | CSSRGBAColor
  | CSSHexColor;

export type NormalizedStroke = {
  colors: NormalizedFill[];
...
function translateScaleMode(
  ...
  css: Partial<NormalizedImageFill>;
  processing: NonNullable<NormalizedImageFill["imageDownloadArguments"]>;
} {
...
): NonNullable<NormalizedImageFill["imageDownloadArguments"]> {
...
/**
 * Build normalized stroke information from a Figma node
 *
 ...
 * @returns Normalized stroke object with colors and properties
 */
export function buildNormalizedStrokes(
  n: FigmaDocumentNode,
  hasChildren: boolean = false,
): NormalizedStroke {
  let strokes: NormalizedStroke = { colors: [] };
  ...
  if (hasValue("strokeWeight", n) && typeof n.strokeWeight === "number" && n.strokeWeight > 0) {
    strokes.border = `${n.strokeWeight}px`;
  }
  ...
  if (hasValue("individualStrokeWeights", n, isStrokeWeights)) {
    const { top, right, bottom, left } = n.individualStrokeWeights;
    const allEqual = top === right && right === bottom && bottom === left;

    if (allEqual && top > 0) {
      strokes.border = `${top}px`;
    } else {
      delete strokes.border;
      if (top > 0) strokes.borderTop = `${top}px`;
      if (right > 0) strokes.borderRight = `${right}px`;
      if (bottom > 0) strokes.borderBottom = `${bottom}px`;
      if (left > 0) strokes.borderLeft = `${left}px`;
    }
  }

  if (
    hasValue("strokeAlign", n) &&
    typeof n.strokeAlign === "string" &&
    n.strokeAlign !== "INSIDE"
  ) {
    strokes.strokeAlign = n.strokeAlign as NormalizedStroke["strokeAlign"];
  }
...
/**
 * Convert a Figma paint (solid, image, gradient) to a NormalizedFill
 * @param raw - The Figma paint to convert
 * @param hasChildren - Whether the node has children (determines CSS properties)
 * @returns The converted NormalizedFill
 */
export function parsePaint(raw: Paint, hasChildren: boolean = false): NormalizedFill {
  if (raw.type === "IMAGE") {
    const baseImageFill: NormalizedImageFill = {
      type: "IMAGE",
 ...
 * @returns The converted pattern NormalizedFill
 */
function parsePatternPaint(
  raw: Extract<Paint, { type: "PATTERN" }>,
): Extract<NormalizedFill, { type: "PATTERN" }> {
  /**
